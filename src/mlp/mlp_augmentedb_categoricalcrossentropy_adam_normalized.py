...
now = datetime.now()
output_file_name = "../../logs/log_mlp_augmentedb_categoricalcrossentropy_adam_normalized_" + str(now)
...
avg_samples = 20
# increasing the number of examples by adding roughly every 'avg_samples' samples a new one
...
while k < len(dummy_x):
    if k == len(dummy_x):
        break
    while i < avg_samples and Y[k] == emotion:
        tmp_array += dummy_x[k]
        i += 1
        k += 1
        if k == len(dummy_x):
            break
    if i >= avg_samples:
        new_x.append(tmp_array / avg_samples)
...
json_file_name = "../../models/mlp_augmentedb_categoricalcrossentropy_adam_normalized_model_" + str(now)
...
hdf5_file_name = "../../models/mlp_augmentedb_categoricalcrossentropy_adam_normalized_model_" + str(now)

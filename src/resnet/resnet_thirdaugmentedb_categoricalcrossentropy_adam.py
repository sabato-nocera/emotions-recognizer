...
now = datetime.now()
output_file_name = "../../logs/log_resnet_thirdaugmentedb_categoricalcrossentropy_adam_refined_" + str(now)
...
def kfold_cross_validation(X_train_kfold, X_test_kfold, y_train_kfold, y_test_kfold):
    ...
    for train, test in kfold.split(inputs, targets):
        # create model
        model = resnet_v1(just_once=1, input_shape=(n_header, 1), depth=20)
...
json_file_name = "../../models/resnet_thirdaugmentedb_categoricalcrossentropy_adam_refined_model_" + str(now)
...
hdf5_file_name = "../../models/resnet_thirdaugmentedb_categoricalcrossentropy_adam_refined_model_" + str(now)

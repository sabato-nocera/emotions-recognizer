...
now = datetime.now()
output_file_name = "../../logs/log_cnn_augmented_categoricalcrossentropy_adam_" + str(now)
...
# increasing the number of examples by adding roughly every two samples a new one
# which is the average of the previous two
emotion = Y[0]
new_x = []
new_y = []
tmp_array = dummy_x[0]
...
print("Other x for train:", len(new_x))
print("Other y for train:", len(new_y))

X_train, X_test, y_train, y_test = train_test_split(dummy_x, dummy_y, test_size=0.2)

X_train = numpy.concatenate([X_train, new_x])

encoder = LabelEncoder()
encoder.fit(new_y)
encoded_new_y = encoder.transform(new_y)
dummy_new_y = np_utils.to_categorical(encoded_new_y)
y_train = numpy.concatenate([y_train, dummy_new_y])

# shuffling thanks to the fact that numpy arrays can be indexed with other arrays
tmp_index = numpy.arange(X_train.shape[0])
numpy.random.shuffle(tmp_index)
...
def kfold_cross_validation(X_train_kfold, X_test_kfold, y_train_kfold, y_test_kfold):
    ...
    print("\nK-fold_Fit: epochs=192, batch_size=32, verbose=2, shuffle=False, validation_split=0.20")
...
json_file_name = "../../models/cnn_augmented_categoricalcrossentropy_adam_model_" + str(now)
...
hdf5_file_name = "../../models/cnn_augmented_categoricalcrossentropy_adam_model_" + str(now)

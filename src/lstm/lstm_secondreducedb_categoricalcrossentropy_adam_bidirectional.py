...
now = datetime.now()
output_file_name = "../../logs/log_lstm_secondreducedb_categoricalcrossentropy_adam_bidirectional_" + str(now)
...
def baseline_model(just_once=0):
    # create model
    model = Sequential()
    model.add(Bidirectional(LSTM(256, input_shape=(X_train2.shape[1], X_train2.shape[2]))))
...
def kfold_cross_validation(X_train_kfold, X_test_kfold, y_train_kfold, y_test_kfold):
    ...
    print("\nK-fold_Fit: epochs=220, batch_size=35, verbose=2, shuffle=False, validation_split=0.20")
    for train, test in kfold.split(inputs, targets):
        ...
        # Fit data to model
        model.fit(inputs[train], targets[train], epochs=220, batch_size=35, verbose=2, shuffle=False,
...
history = keras_model.fit(X_train2, y_train, epochs=220, batch_size=35, verbose=2, shuffle=False,
                          validation_split=0.20, )
print("\nFit: epochs = 220, batch_size = 35, verbose = 2, shuffle=False, validation_split = 0.20\n")
...
json_file_name = "../../models/lstm_secondreducedb_categoricalcrossentropy_adam_bidirectional_model_" + str(now)
...
hdf5_file_name = "../../models/lstm_secondreducedb_categoricalcrossentropy_adam_bidirectional_model_" + str(now)
